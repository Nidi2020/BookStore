...
public static class ModelBuilderExtensions
{
    ...
    public static void AddRestrictDeleteBehaviorConvention(this ModelBuilder modelBuilder)
    {
        ...
        foreach (IMutableForeignKey fk in cascadeFKs)
            fk.DeleteBehavior = DeleteBehavior.Restrict; //default add mikonim--yani:
                                                         //age pedari havi chand ta farzand bod... pedar ghabele hazf shodan nabashe!
                                                         //children avval hazf beshe bad pedar
    }
    ...
    {
        IEnumerable<Type> types = assemblies.SelectMany(a => a.GetExportedTypes()) // type haei ke az biroon ghabele dastras hastan mesle public classes
            .Where(c => c.IsClass && !c.IsAbstract && c.IsPublic && typeof(BaseType).IsAssignableFrom(c));

        foreach (Type type in types)
            modelBuilder.Entity(type); // be context morefi mikone va table misaze
    }
}
